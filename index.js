...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `shanz Bot Ofc.`, "footerText": "*_SHANZ-BOT_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`SHANZ-BOT 😎`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = xeon.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*shanz*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             xeon.relayWAMessage(res)
        }
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `shanz`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/94725292890?text=hello`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nSHANZ-BOT😎. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
*Thank you for registering in the SHANZ-BOT😎 Database*
...
*「 SHANZ-BOT 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              ...
              buttonsMessage = {footerText:'ꜱʜᴀɴᴢ', imageMessage: imageMsg,
	    ...
	    // console.log(e)
			...
			reply(`*Thank you for being registered SHANZ-BOT*`)
		}, 2000)
        break
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =` ▢ 𝐇𝐢 ${pushname} \n ▢ 𝐆𝐫𝐨𝐮𝐩 𝐂𝐡𝐚𝐭𝐬 : ${groups.length} \n ▢ 𝐏𝐫𝐢𝐯𝐚𝐭𝐞 𝐂𝐡𝐚𝐭𝐬 : ${privat.length}\n ▢ 𝐓𝐨𝐭𝐚𝐥 𝐂𝐡𝐚𝐭𝐬 : ${totalChat.length} \n ▢ 𝐒𝐩𝐞𝐞𝐝 : ${latensie.toFixed(4)} _Second_ \n ▢ 𝐀𝐜𝐭𝐢𝐯𝐞 : ${runtime(process.uptime())} \n ▢ 𝐏𝐥𝐚𝐭𝐟𝐨𝐫𝐦 : ${os.platform()}`
               ...
               buttonsMessage = {
               contentText: `${txtt}`,
               footerText: `SHANZ-BOT`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
prep = await xeon.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
...
│
│  ❑ *ɢɪᴛʜᴜʙ ʟɪɴᴋ* :
│  https://github.com/MSHANZ1/Shanzbot
│  
│  ❑ *ʏᴏᴜᴛᴜʙᴇ ʟɪɴᴋ* :
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               ...
               buttons = [{buttonId: `mygithub`,buttonText:{displayText: 'SC BOT'},type:1},{buttonId:`iggw`,buttonText:{displayText:'CREATOR'},type:1}]
...
`
sendButMessage(from, menu, `SHANZ-BOT`, [
          {
            buttonId: `command`,
            buttonText: {
              displayText: `Back Menu ︎`,
            },
            type: 1,
          },]);
...
break
//fun features by shanz
                case 'uglycheck':
					// source code by 🅼『SHANZ』亗
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  sange = body.slice(1)
...
case "intake3": // by shanz
      ...
      case "group":
        if (!mek.key.fromMe) return;
        sendButMessage(from, `GROUP SETTING`, `Please choose one/nꜱʜᴀɴᴢ `, [
          {
            buttonId: `unmute`,
            buttonText: {
              displayText: `OPEN`,
...
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              ...
              buttonsMessage = {footerText:'SHANZ-BOT', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, `SHANZ-BOT`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 SHANZ-BOT BROADCASTING 」*\n\n${body.slice(4)}`})
}
...
case 'writelist':
  ...
  ▢ ${prefix}write shanz
  ▢ ${prefix}folioright shanz
  ▢ ${prefix}folioleft shanz
  ▢ ${prefix}writedown shanz
  ▢ ${prefix}writeleft shanz`)
  break
case 'folioleft':
if (!c) return reply('Where is the text bro??')
reply(mess.wait)
...
case 'nsfw':
			...
			case 'quote': 
					...
					buffer1 = await getBuffer(anu.result.url)
buttons = [{buttonId: `quote`,buttonText:{displayText: `Thx already used`},type:1}]
...
case 'waifu':
  ...
  break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} shanz`)
